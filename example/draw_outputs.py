...
def plot_distribution(out, leaf='F1', variable='necrosis_percentage', ylabel='% Septoria necrotic'):
    fig, ax = plt.subplots(1,1)
    for i_rep, out_rep in out.items():
        data = pandas.DataFrame({pl:v[leaf].data[variable].values for pl,v in out_rep.items()},
...
def plot_by_plant(out, param_name='Fraction spo', variable = 'leaf_disease_area', group_by=None,
    ...
    def customize_ax(ax, param, title):
        if list(od.keys()).index(param)==len(list(od.keys()))-1:
            ax.set_xlabel('Degree days', fontsize=18)
        ax.set_ylabel(ylabel, fontsize=18)
    ...
    od = OrderedDict(sorted(list(out.items()), key=lambda x: x[0]))
    h={}
    if group_by!='plant':
        if axs==None:
            fig, axs = plt.subplots(len(list(od.keys())),len(plants))
        if group_by==None:
            for param, out_par in od.items():
                for i_rep, out_par_rep in out_par.items():
                    for pl in plants:
                        for lf in leaves:
                            reco = out_par_rep[pl][lf]
                            if len(list(od.keys()))>1:
                                ax = axs[list(od.keys()).index(param)][plants.index(pl)]
                            else:
        ...
        if group_by=='repetition' or group_by=='leaf':
            for param, out_par in od.items():
                for pl in plants:
                    ax = axs[list(od.keys()).index(param)][plants.index(pl)]
                    for lf in leaves:
                        data = pandas.DataFrame({k:v[pl][lf].data[variable].values for k,v in out_par.items()},
                                                index=out_par[list(out_par.keys())[0]]['P1']['F10'].data.degree_days)
        ...
        if axs == None:
            fig, axs = plt.subplots(1, len(list(od.keys())))
        if not is_iterable(axs):
            axs = numpy.array([axs])
        for param, out_par in od.items():
            ax = axs[list(od.keys()).index(param)]
            for lf in leaves:
                for pl in plants:
                    data = pandas.DataFrame({k:v[pl][lf].data[variable].values for k,v in out_par.items()},
                                            index=out_par[list(out_par.keys())[0]]['P1']['F10'].data.degree_days)
    ...
    if group_by!='leaf':
        ...
        if len(list(od.keys()))>1 and group_by!='plant':
...
def plot_by_state(out, param_name='Fraction spo', plant='P1',
    ...
    cmapdict = {leaves[i]:cmap(int(indcolors[i])) for i in range(len(indcolors))}
    od = OrderedDict(sorted(list(out.items()), key=lambda x: x[0]))
    fig, axs = plt.subplots(6, len(list(od.keys())))
    h = {}
    ...
    for param, out_par in od.items():
        for state, variable in states.items():
            ax = axs[state][list(od.keys()).index(param)]
            for lf in leaves:
                data = pandas.DataFrame({k:v[plant][lf].data[variable].values for k,v in out_par.items()},
                                        index=out_par[list(out_par.keys())[0]][plant]['F10'].data.degree_days)
                h[lf] = ax.plot(data.mean(axis=1).index, data.mean(axis=1).values, color=cmapdict[lf])
            xmax = max(data.mean(axis=1).index)
            
            if state==list(states.keys())[-1]:
                ax.set_xlabel('Degree days', fontsize=16)
            if param==list(od.keys())[0]:
