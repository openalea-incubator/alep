...
PopMyc duplicatePopMyc(PopMyc *source) {
  PopMyc pop;
  pop.type = source->type;
  pop.nbClass = source->nbClass;
  pop.agemin = source->agemin;
  pop.agemax = source->agemax;
  pop.Q = (float*) calloc(pop.nbClass, sizeof(float));
  pop.N = (float*) calloc(pop.nbClass, sizeof(float));
  pop.decT = (float*) calloc(pop.nbClass, sizeof(float));
  pop.largClass = source->largClass;
  pop.pC0 = source->pC0;
  pop.TimeBuffer = source->TimeBuffer;
  return(pop);
}
