...
def leaf_wetness_rapilly(rain=0., relative_humidity=0., PPFD=0.):
    ...
    """
    if isinstance(rain,Iterable):
        return list(map(lambda x:leaf_wetness_rapilly(*x), zip(rain, relative_humidity, PPFD)))
    else:
        if rain>0. or (relative_humidity >= 85. and PPFD < 644.):
            return True
        else:
            return False
