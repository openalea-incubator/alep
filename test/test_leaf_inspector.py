...
def generate_lesions_with_surfaces(nb_lesions = 100, surface_inc = 0.1, surface_chlo = 0.2,
    ...
    for lesion in lesion_stock:
        ...
        #lesion.surfaces_inc = surface_inc
        lesion.surfaces_chlo = np.append(lesion.surfaces_chlo, surface_chlo)
        lesion.surfaces_nec = np.append(lesion.surfaces_nec,surface_nec)
        lesion.surfaces_spo = np.append(lesion.surfaces_spo, surface_spo)
        assert lesion.necrotic_area == necrotic_area_new(lesion)
    return lesion_stock
